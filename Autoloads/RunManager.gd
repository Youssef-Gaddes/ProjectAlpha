...
func _filter_maps(zone: MapData.ZoneCategory, node_index: int, used: Dictionary) -> Array[MapData]:
	...
	for map_data in all_maps:
		...
		var req_result = map_data.meets_requirements()
		if not map_data.meets_requirements():
			continue
		print("[Node %d] %s → meets_requirements: %s | knights opinion: %d" % [node_index, map_data.map_id, req_result, Stats.faction_opinions.get("knights", 0)])
...
func _select_map_weighted(options: Array[MapData]) -> MapData:
	...
	for i in options:
		if pick == null:
			pick = i
		elif i.weight>pick.weight:
			pick = i
		elif i.weight == pick.weight:
			var rand = [i, pick].pick_random()
			pick = rand
...
func _load_node(node_index: int) -> void:
	...
	if map_data.possible_scenes.is_empty():
		push_error("MapData %s has no scenes defined" % map_data.map_id)
		return

	var scene_path: String = map_data.possible_scenes.pick_random()
