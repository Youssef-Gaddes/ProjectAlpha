...
@onready var circle: PackedScene = preload("res://entities/player/God/Judge/await_judgement.tscn")
...
# === INITIALIZATION ===
func _ready():
	get_tree().get_first_node_in_group('UI').lock_player.connect(_on_ui_lock_started)
	get_tree().get_first_node_in_group('UI').unlock_player.connect(_on_ui_lock_ended)
...
func _on_ui_lock_started():
	locked = true
	_play_animation('Idle')
	
func _on_ui_lock_ended():
