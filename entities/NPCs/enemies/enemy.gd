...
func _state_windup(_delta: float) -> void:
	...
	if current_attack.attack_name == "Lunge Strike":
...
func _state_attacking(_delta: float) -> void:
	...
	if current_attack.has_lunge == true:
...
func _on_attack_complete() -> void:
	...
	if current_attack.has_lunge:
		npc.set_collision_mask_value(3, true)
	
	# Start cooldown
	attack_cooldown_timer = current_attack.attack_cooldown
